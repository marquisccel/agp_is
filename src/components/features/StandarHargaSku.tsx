...
export type StandarHarga = { sku_name: string; max_price_per_kg: number }

/**
 * Keterangan standar harga SKU di bawah kolom harga.
 *
 * Kenapa ada: standar harga per SKU ditetapkan Manager per gudang, dan
 ...
 * standarnya, lalu notanya tertahan; Admin menyimpan verifikasi dan baru
 ...
 * penyimpanan. Harga di atas standar tetap boleh dicatat, karena memang ada
 ...
 * sesudahnya.
 *
 * Dipakai di dua form (Input Pembelian dan Double Check) supaya angka dan
 * kata-katanya tidak berangsur berbeda di dua layar.
 */
export function standarHargaSku(skuName: string, standar: StandarHarga[]): number | null {
  if (!skuName) return null
  const cocok = standar.find((s) => s.sku_name === skuName)
  return cocok ? cocok.max_price_per_kg : null
}

export default function StandarHargaSku({
  skuName,
  harga,
  standar,
}: {
  skuName: string
  harga: number
  standar: StandarHarga[]
}) {
  const batas = standarHargaSku(skuName, standar)

  // SKU yang belum punya standar tidak diberi keterangan apa pun: kontrol
  // harga memang tidak aktif untuknya, dan menulis "belum ada standar"
  // hanya menambah baris yang tidak menuntun ke mana-mana.
  ...
  return (
    <p className="mt-1.5 text-[11px] font-semibold" style={{ color: melebihi ? "var(--warning)" : "var(--muted-faint)" }}>
      {melebihi
        ? `Di atas standar ${fmtRp(batas)}/kg. Nota akan menunggu persetujuan Manager.`
        : `Standar harga ${fmtRp(batas)}/kg`}
    </p>
  )
}
