...
| API route smoke test | Done (2026-08-09, diwire ke CI 2026-08-11) | `smoke/api-smoke.mjs` (`npm run test:smoke`) -- health check, akses tanpa sesi ditolak, kontrol peran (Manager tidak bisa ajukan kasbon, Staff tidak bisa double-check), siklus penuh draft->double-check->transfer, dan (sejak 2026-08-11) refund DP saat transaksi dihapus permanen. Data uji dibuat dan dibersihkan sendiri (tidak hardcode ID). Ketemu 1 bug nyata: DELETE transaksi manager tidak menghapus berkas bukti transfer dari disk -- sudah diperbaiki. Sekarang jalan otomatis di CI tiap push/PR lewat `.github/workflows/ci.yml` (service Postgres + migrate + seed). |
...
Fase 7 dapat dianggap **selesai dari sisi perangkat/tooling**, sama seperti status penutupan Fase 6. PRD diperbarui ke v1.3 lalu v1.4 untuk mencatat penutupan ini.

### Follow-up pasca-Fase 7 (2026-08-11)

Di luar cakupan checklist Fase 7 di atas, tiga hal lagi diselesaikan sebagai audit lanjutan terhadap register isu PRD dan utang teknis:

- **Audit D-1 s.d. D-4 (register isu prioritas tinggi, PRD Bagian 14.1)** -- D-1, D-3, D-4 ternyata sudah diperbaiki pada pekerjaan sebelumnya (diverifikasi ulang lewat kode, bukan cuma dipercaya dari komentar). D-2 ditemukan 1 celah tambahan sekelas lewat jalur berbeda: `DELETE /api/manager/purchases/[id]` (hapus transaksi permanen) tidak mengembalikan DP yang terpakai -- sudah diperbaiki (`refundDp` dibungkus `prisma.$transaction`) dan ditambah cakupan smoke test.
- **D-18 (utang teknis, daftar libur nasional hardcode)** -- dipindah ke `src/data/national-holidays.json`, plus warning terstruktur otomatis kalau kode dipanggil untuk tahun yang datanya belum ada (supaya kondisi data basi kepantau, bukan gagal diam-diam).
- **CI wiring** -- `.github/workflows/ci.yml` sekarang menjalankan smoke test dan Playwright secara otomatis tiap push/PR (service Postgres, migrate, seed, build, start server, lalu kedua suite), tidak lagi cuma manual. Proses ini juga menemukan dan menutup 2 gap tersembunyi yang lolos dari verifikasi manual sebelumnya: `package-lock.json` yang drift dari platform Windows (bikin `npm ci` gagal di Linux runner) dan 1 selector Playwright yang rapuh terhadap perbedaan tinggi render form antar environment (dropdown ElegantSelect posisinya di luar viewport headless Ubuntu, walau lolos mulus di lokal).

### Fase 8 - UI/UX Premium Pass (dimulai 2026-08-11)

Audit menyeluruh terhadap pacing/feedback/disabled state/confirmation/empty state/error state di seluruh alur (input pembelian, double check, edit transaksi, nota, DP/kasbon, master data). Sebagian besar area yang diaudit (loading/disabled state di semua form utama, empty state di semua list, error banner) ternyata sudah rapi dari pekerjaan sebelumnya -- gap nyata yang ditemukan dan diperbaiki:

| Area | Status | Notes |
| --- | --- | --- |
| `ExpenseAnalytics` & `SusutLebihAnalytics` dead code | Done (2026-08-11) | Dua temuan lama dari Fase 5: `ExpenseAnalytics` selalu render kosong (filter exclude "kediri/malang/madiun" kebalik logikanya untuk 3 gudang asli yang ada) -- filter dibuang, ditambah kartu ringkasan global. Prop `summary` di `SusutLebihAnalytics` dead code sungguhan (komponen sudah hitung `filteredSummary` sendiri yang lebih berguna) -- dihapus di 2 caller. |
| Konfirmasi aksi konsisten | Done (2026-08-11) | `src/components/ui/ConfirmDialog.tsx` (hook `useConfirm()`) menggantikan `window.confirm()` bawaan browser di 6 titik (hapus transaksi, hapus lapak, tolak/forward DP, tolak harga, ganti bukti transfer) -- konsisten dengan pola custom banner yang sudah ada di SupplierForm untuk peringatan duplikat. Ditambah 2 konfirmasi yang sebelumnya hilang sama sekali (tolak harga, ganti bukti). |
| Notifikasi konsisten | Done (2026-08-11) | `src/components/ui/Toast.tsx` (hook `useToast()`) menggantikan `window.alert()` di 10 titik. Migrasi ini menemukan 1 regresi tersembunyi: `ManagerSuppliersClient` menyalin prop ke `useState` lokal, jadi `router.refresh()` saja tidak menghapus baris dari tampilan setelah delete -- diperbaiki dengan filter state lokal langsung, ketemu lewat verifikasi manual di browser (bukan cuma baca kode). |
| Bug sesi basi (stale JWT) | Done (2026-08-11) | Ditemukan lewat laporan pengguna: ganti nama profil gagal dengan pesan "email sudah digunakan" yang salah, saat sesi JWT menunjuk ke baris User yang sudah tidak ada (mis. setelah DB direset ulang sementara pengguna masih login). `PUT /api/user/settings` sekarang validasi baris user ada dulu, kasih pesan jelas ("sesi tidak valid, keluar dan masuk kembali") ketimbang salah diagnosis. Rute lain yang pakai `session.user.id` serupa (16 file) sudah dibungkus try/catch generik -- tidak crash mentah, hanya belum punya pesan sejelas ini; risiko rendah karena kondisi pemicu (DB direset saat sesi aktif) jarang terjadi di luar environment testing. |
| Audit sisa halaman (staff DP, master-data) | Done (2026-08-11) | `DPRequestForm`, halaman kasbon staff, dan `MasterDataClient` sudah punya loading/disabled/empty state yang baik sejak awal -- tidak ada aksi destruktif di `MasterDataClient` (murni tampilan). Tidak ada perubahan diperlukan. |

Item yang masih terbuka murni menunggu keputusan produk/stakeholder, bukan pekerjaan teknis: lihat Bagian 16.3 PRD (pertanyaan A-1 s.d. E-2) dan keputusan `ExpenseAnalytics` (summary card vs dihapus -- sudah diputuskan jadi summary card, lihat baris di atas).
